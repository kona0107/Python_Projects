...
def fetch_data(start_date, end_date):
    """데이터를 한 번에 가져오는 함수."""
    global desired_dms_codes
    
    # 엑셀 파일 읽기
    file_path = config.DMS_CODE_PATH
...
def adjust_date_for_nighttime(data):
    ...
    data['측정일'] = data['date'].dt.date  # 날짜만 추출
...
def calculate_monthly_and_yearly_averages(data):
    ...
    월평균_기온 = data.groupby(['month', 'DMS_CODE'])['TMP'].mean().reset_index()
    월평균_기온.rename(columns={'TMP': 'Temp_1_monthly_mean'}, inplace=True)

    # 연평균 온도 계산
    data['year'] = data['측정일'].dt.year  # 연도 추출
    년평균_기온 = data.groupby(['year', 'DMS_CODE'])['TMP'].mean().reset_index()
    년평균_기온.rename(columns={'TMP': 'Temp_1_yearly_mean'}, inplace=True)

    # 결과 병합
    data = pd.merge(data, 월평균_기온, on=['month', 'DMS_CODE'], how='left')
    data = pd.merge(data, 년평균_기온, on=['year', 'DMS_CODE'], how='left')
    
    return data


def fetch_and_process_all_years(df):
    ...
    full_data = pd.concat([df, aggregated_data], ignore_index= True)
    
    full_data.to_excel(config.BASE_DIR + f"/machine_learning_project/data/weather/2015_2024_daily_data.xlsx", index=False)

    # 결측치 처리: 1순위, 2순위, 3순위, 4순위 순으로 처리
    filled_data = process_and_fill_gaps(full_data)

    # 최종 데이터 처리
    final_data = calculate_monthly_and_yearly_averages(filled_data)
    
    final_data.drop(columns=['month', 'year'], inplace=True, errors='ignore')
    
    # 열 이름 설정 및 순서 재정렬
    colname = ['측정일', 'DMS_CODE', '측정소명', 'remark','TMP', 'TMAX', 'TMIN',
               'PCP', 'HUM', 'WS', 'Temp_1_monthly_mean', 'Temp_1_yearly_mean']
    
    final_data.columns = colname
    
    final_data = final_data[['측정일', 'DMS_CODE','측정소명', 'TMP', 'TMAX', 'TMIN', 'PCP', 'HUM', 'WS', 'Temp_1_monthly_mean', 'Temp_1_yearly_mean', 'remark']]

    # 결과 저장
    final_data.to_excel(config.BASE_DIR + "/machine_learning_project/data/weather/2015_2024_mosquito.xlsx", index=False)
    
    print("All data processing complete. Saved to '2015_2024_mosquito.xlsx'.")
...
if __name__ == "__main__":
    ...
    final_processed_data = fetch_and_process_all_years(df_2)